...
public class DOM2Writer {
    ...
    private static void print(final Node node, final NSStack namespaceStack, final Node startnode, final PrintWriter out,
        ...
        switch (type) {
            ...
            case Node.ELEMENT_NODE:
                ...
                if (children3 != null) {
                    int numChildren = children3.getLength();
                    ...
                    for (int i = 0; i < numChildren; i++) {
                        print(children3.item(i), namespaceStack, startnode, out, pretty, indent + 1, encoder);
                    }
                } else {
                    hasChildren = false;
                }
     ...
     *        the out
     */
    private static void printNamespaceDecl(final Element owner, final Node node, final NSStack namespaceStack,
     ...
     */
    private static void printNamespaceDecl(final Node node, final NSStack namespaceStack, final Node startnode,
            final PrintWriter out) {
        switch (node.getNodeType()) {
            case Node.ATTRIBUTE_NODE:
                printNamespaceDecl(((Attr) node).getOwnerElement(), node, namespaceStack, startnode, out);
                break;
    ...
    private static void visit(final Node node, final int level, final String encoding) {
        ...
        for (int i = 0; i < list.getLength(); i++) {
            ...
            // System.out.println("node type is " + childNode.getNodeType() + "
            // --- "
            ...
            if (childNode.getNodeType() == Node.PROCESSING_INSTRUCTION_NODE) {
                ...
                if ("xml".equals(pi.getTarget()) && pi.getData().startsWith("encoding")) {
                    pi.getData().substring(pi.getData().indexOf("\"") + 1, pi.getData().lastIndexOf("\""));
                    return;
                }
            }
            visit(childNode, level + 1, encoding);
        }
    }
}
