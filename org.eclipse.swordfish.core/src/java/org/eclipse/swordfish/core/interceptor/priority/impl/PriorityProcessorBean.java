 ...
 ******************************************************************************/
package org.eclipse.swordfish.core.interceptor.priority.impl;
...
/**
 * The Class PriorityProcessorBean.
 */
public class PriorityProcessorBean extends AbstractProcessingComponent implements PriorityProcessor {

    /** The Constant log. */
    private final static Log LOG = SBBLogFactory.getLog(PriorityProcessorBean.class);
    ...
    private void handle(final MessageExchange context, final PrimitiveAssertion assertion, final ParticipantRole participantRole) {
        ...
        if (null != value) {
            try {
                context.setProperty(ExchangeProperties.PRIORITY, new Integer(value));
                LOG.debug("Setting priority to " + value);
            } catch (NumberFormatException e) {
                LOG.warn("Illegal value specified for Priority: " + value);
