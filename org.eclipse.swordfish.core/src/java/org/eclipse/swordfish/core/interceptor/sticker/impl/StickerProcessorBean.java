...
import org.eclipse.swordfish.core.interceptor.sticker.StickerProcessor;
...
/**
 * The Class StickerProcessorBean.
 */
public class StickerProcessorBean extends AbstractProcessingComponent implements StickerProcessor {
    ...
    public void handleRequest(final MessageExchange context, final Role role, final Collection/* <Assertion> */assertions)
        ...
        if (role.equals(Role.SENDER)) {
            // this is the only path we have a sense making usage of this
            // assertion. We will not put anything on distributet conversations,
            // so we always switch for those.
            if ("single".equals("bla"/* assertion.getContent().getAttribute("support") */)) {
                KeyObject ko =
                        new KeyObject(HeaderUtil.getCallContextExtension(context).getUnifiedParticipantIdentity()
                            .getParticipantIdentity(), context.getService());
                context.setProperty(ExchangeProperties.CONVERSATION_INDTIFIER, ko);
                context.setProperty(ExchangeProperties.CONVERSATION_REFRESHABLE, new Boolean(false));
            }
        }
    }
