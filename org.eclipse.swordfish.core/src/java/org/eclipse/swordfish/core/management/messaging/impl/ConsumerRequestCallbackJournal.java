...
/**
 * The Class ConsumerRequestCallbackJournal.
 * 
 */
public class ConsumerRequestCallbackJournal extends ConsumerJournal {

    /**
     * Instantiates a new consumer request callback journal.
     * 
     * @param notification
     *        the notification
     */
    public ConsumerRequestCallbackJournal(final MessageProcessingNotification notification) {
        super(notification);
    }
     ...
     *        the scaffold
     */
    protected ConsumerRequestCallbackJournal(final ConsumerRequestCallbackJournal scaffold) {
        super(scaffold);
    }
    ...
    public void process(final MessageProcessingNotification notification) {
        ...
        if (ExchangeState.ACTIVE.equals(notification.getExchangeState())) {
            if (EventType.APP_IN_PRE.equals(notification.getEventType())) {
                if (null != this.getLastEvent()) {
                    this.handleSequenceError(notification);
                }
                this.handleStarted(notification);
            ...
            } else if (EventType.NET_IN_PRE.equals(notification.getEventType())) {
    ...
    public void replay(final MessagingMonitorBackend backend) {
        if ((null != this.getNotifications()) && (this.getNotifications().size() > 0)) {
            ExchangeJournal tempJournal = new ConsumerRequestCallbackJournal(this);
