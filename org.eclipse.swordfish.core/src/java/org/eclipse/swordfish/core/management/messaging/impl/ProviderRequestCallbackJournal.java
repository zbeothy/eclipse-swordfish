...
/**
 * The Class ProviderRequestCallbackJournal.
 * 
 */
public class ProviderRequestCallbackJournal extends ProviderJournal {

    /**
     * Instantiates a new provider request callback journal.
     * 
     * @param notification
     *        the notification
     */
    public ProviderRequestCallbackJournal(final MessageProcessingNotification notification) {
        super(notification);
    }
     ...
     *        the scaffold
     */
    protected ProviderRequestCallbackJournal(final ProviderRequestCallbackJournal scaffold) {
        super(scaffold);
    }
    ...
    public void process(final MessageProcessingNotification notification) {
        ...
        if (ExchangeState.ACTIVE.equals(notification.getExchangeState())) {
            if (EventType.NET_IN_PRE.equals(notification.getEventType())) {
            ...
            } else if (EventType.NET_IN_POST.equals(notification.getEventType())) {
                ...
                if (!EventType.NET_IN_POST.equals(this.getLastEvent())) {
                    this.handleSequenceError(notification);
                }
                this.handleHandoff(notification);
            } else if (EventType.APP_IN_PRE.equals(notification.getEventType())) {
                if (!EventType.APP_OUT_PRE.equals(this.getLastEvent())) {
                    this.handleSequenceError(notification);
                }
                this.handleEvent(notification);
            } else if (EventType.APP_IN_POST.equals(notification.getEventType())) {
                if (!EventType.APP_IN_PRE.equals(this.getLastEvent())) {
                    this.handleSequenceError(notification);
                }
                this.handleHandback(notification);
            } else if (EventType.NET_OUT_PRE.equals(notification.getEventType())) {
                if (!EventType.APP_IN_POST.equals(this.getLastEvent())) {
                    this.handleSequenceError(notification);
                }
                this.handleEvent(notification);
            } else if (EventType.NET_OUT_POST.equals(notification.getEventType())) {
                if (!EventType.NET_OUT_PRE.equals(this.getLastEvent())) {
                    this.handleSequenceError(notification);
                }
                this.handleDelivered(notification);
            } else if (EventType.APP_OUT_POST.equals(notification.getEventType())) {
                if (!EventType.NET_OUT_POST.equals(this.getLastEvent())) {
                    this.handleSequenceError(notification);
                }
                this.handleEvent(notification);
            } else {
            ...
            this.handleDoneLocal(notification);
    ...
    public void replay(final MessagingMonitorBackend backend) {
        if ((null != this.getNotifications()) && (this.getNotifications().size() > 0)) {
            ExchangeJournal tempJournal = new ProviderRequestCallbackJournal(this);
