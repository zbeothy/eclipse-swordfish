...
/**
 * The Class OutOnlyMonitorBackend.
 */
public class OutOnlyMonitorBackend extends OperationMonitorBackend {

    /** The Constant log. */
    private final static Log LOG = SBBLogFactory.getLog(OutOnlyMonitorBackend.class);

    /** The history. */
    private InOnlyHistory history;

    /**
     * Instantiates a new out only monitor backend.
     * 
     * @param participant
     *        the participant
     * @param serviceName
     ...
     */
    public OutOnlyMonitorBackend(final UnifiedParticipantIdentity participant, final QName serviceName, final String opName,
    ...
    public void publishSummary() {
        ...
        if (0 != oldHistory.getCalls()) {
            Object[] params =
                    {String.valueOf(this.getService()), this.getOperationName(), String.valueOf(this.getRole()),
